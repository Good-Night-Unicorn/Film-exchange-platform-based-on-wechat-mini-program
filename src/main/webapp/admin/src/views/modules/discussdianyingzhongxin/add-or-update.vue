...
export default {
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `discussdianyingzhongxin/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `discussdianyingzhongxin/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.discussdianyingzhongxinCrossAddOrUpdateFlag = false;
						 this.parent.search();
