...
export default {
  ...
  methods: {
	...
	this.ruleForm.dianyingjianjie = this.ruleForm.dianyingjianjie.replace(reg,'../../../ssm2khae/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `dianyingzhongxin/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.dianyingzhongxinCrossAddOrUpdateFlag = false;
						 this.parent.search();
